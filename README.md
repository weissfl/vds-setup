...
Меняет порт SSH (пример — `2294`, можно указать любой свободный порт):
```
Port 2294
```
...
Параметры управления доступом к SSH (запрет root-входа и паролей); строки нужно изменить или добавить:
```
PermitRootLogin no
...
Добавляет настройки для защиты SSH с указанием нового порта:
```ini
[sshd]
enabled = true
port = 2294
