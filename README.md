...
Автоматически копирует публичный ключ в буфер обмена — останется только вставить его на сервере:
```powershell
...
**Параметры:**
- `maxretry = 3` — количество неудачных попыток перед баном
- `bantime = 3600` — время бана в секундах (1 час)
- `findtime = 600` — окно времени для подсчёта попыток (10 минут)
